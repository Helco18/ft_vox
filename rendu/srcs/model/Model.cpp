...
Model::Model(const std::string & path)
{
	...
	while (std::getline(file, tmp))
	{
		...
		{
			glm::vec3 position(std::atof(&(splitted[0].c_str())[2]), std::atof(splitted[1].c_str()), std::atof(splitted[2].c_str()));
			glm::vec3 color(std::atof(splitted[3].c_str()), std::atof(splitted[4].c_str()), std::atof(splitted[5].c_str()));
			glm::vec2 texCoord(std::atof(splitted[6].c_str()), std::atof(splitted[7].c_str()));
